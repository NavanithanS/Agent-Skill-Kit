...
def sync_readme_table(skills: list) -> bool:
    """Rewrite the README block between the SKILLS markers.

    The README is the site's root landing page, so these rows are what give
    each per-skill page an internal link from the most-crawled page in the
    repo. Returns False if the markers are absent.
    """
    readme = ROOT / "README.md"
    text = readme.read_text(encoding="utf-8")
    if README_START not in text or README_END not in text:
        print("⚠️  README.md is missing the SKILLS markers — table not synced")
        return False
    if text.index(README_START) > text.index(README_END):
        print("⚠️  README.md SKILLS markers are out of order — table not synced")
        return False

    def row(skill: dict) -> str:
        category = skill.get("category", "other")
        name = skill["name"]
        # Collapse whitespace and escape pipes: a folded YAML description can
        # contain newlines, and either would break the markdown table.
        description = " ".join(skill.get("description", "").split()).replace("|", "\\|")
        return f"| [`{name}`](skills/{category}/{name}/) | {category} | {description} |"

    rows = "\n".join(
        row(s)
        for s in sorted(skills, key=lambda s: (s.get("category", "other"), s["name"]))
    )
    block = (
        f"{README_START}\n"
        f"<!-- Generated by scripts/generate_site.py — do not edit by hand. -->\n\n"
        f"| Skill | Category | What it does |\n|---|---|---|\n{rows}\n\n"
        f"{README_END}"
    )
    ...
    if updated != text:
        ...
        print(f"✅ README skill table synced ({len(skills)} skills)")
    return True


def generate_skill_index(skills: list) -> str:
    """Build skills/README.md — the crawlable hub linking every skill page.

    Jekyll renders this at /skills/, so each per-skill README gains a parent
    with a real anchor pointing at it.
    """
    categories = sorted({s.get("category", "other") for s in skills})
    agents = sorted({a for s in skills for a in s.get("agents", [])})
    count = len(skills)

    lines = [
        "---",
        f'title: "Skill library — {count} agent skills for Claude Code, Codex, Gemini CLI and Cursor"',
        f'description: "Browse all {count} Agent Skill Kit skills across '
        f'{", ".join(categories)}. Each installs to Claude Code, Codex, Gemini CLI, '
        'Cursor or Antigravity with one command."',
        "---",
        "",
        "# Skill library",
        "",
        f"All {count} skills in [Agent Skill Kit](https://navanithans.github.io/Agent-Skill-Kit/), "
        f"across {len(categories)} categories. Every skill installs to any of "
        ...
        "",
        "Prefer a UI? Use the "
        "[interactive command builder](https://navanithans.github.io/Agent-Skill-Kit/docs/).",
        "",
    ]

    for category in categories:
        in_cat = sorted(
            (s for s in skills if s.get("category", "other") == category),
            key=lambda s: s["name"],
        )
        if not in_cat:
            continue
        label = CATEGORY_META.get(category, {}).get("label", category.title())
        lines += [f"## {label} ({len(in_cat)})", ""]
        for skill in in_cat:
            name = skill["name"]
            lines.append(
    ...
    lines += [
        "---",
        "",
        "[Documentation](https://navanithans.github.io/Agent-Skill-Kit/) · "
        "[Command builder](https://navanithans.github.io/Agent-Skill-Kit/docs/) · "
        "[GitHub](https://github.com/NavanithanS/Agent-Skill-Kit) · "
        "[PyPI](https://pypi.org/project/agent-skill-kit/)",
        "",
    ]
    return "\n".join(lines)
...
def generate_index_page(skills: list, generated: str) -> str:
    ...
    # Static, server-rendered skill index. Real <a> tags so crawlers can reach
    # every per-skill page without executing the SKILLS array. Links resolve to
    # the Jekyll-rendered README at skills/<category>/<name>/.
    _static_sections = []
    for c in categories:
        cat_skills = sorted(
            (s for s in skills if s.get("category", "other") == c),
            key=lambda s: s["name"],
        )
        if not cat_skills:
            continue
        label = CATEGORY_META.get(c, {}).get("label", c)
        items = '\n'.join(
            '<li class="py-1.5">'
            f'<a href="../skills/{html.escape(c)}/{html.escape(s["name"])}/" '
            'class="text-sm font-medium text-gray-900 dark:text-zinc-100 hover:underline">'
            f'{html.escape(s["name"])}</a> '
            f'<span class="text-sm text-gray-500 dark:text-zinc-400">— {html.escape(s.get("description", ""))}</span>'
            '</li>'
            for s in cat_skills
        )
        _static_sections.append(
            f'<h3 class="text-sm font-bold uppercase tracking-wide text-gray-500 '
            f'dark:text-zinc-400 mt-6 mb-2">{html.escape(label)} ({len(cat_skills)})</h3>'
            f'<ul class="divide-y divide-gray-100 dark:divide-zinc-800">{items}</ul>'
        )
...
<head>
  ...
  <title>Skill browser &amp; command builder — Agent Skill Kit</title>
  <meta name="description" content="Browse all {len(skills)} Agent Skill Kit skills and generate ask copy / ask purge commands for Claude Code, Codex, Gemini CLI, Cursor and Antigravity.">
  <link rel="canonical" href="https://navanithans.github.io/Agent-Skill-Kit/docs/">
  <meta name="google-site-verification" content="Rfmf9zkKXT4LZoUsDXeP_uAZSkYLVptJF85hHCptfic">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="Agent Skill Kit">
  <meta property="og:title" content="Skill browser &amp; command builder — Agent Skill Kit">
  <meta property="og:description" content="Browse {len(skills)} agent skills and build ask commands for five coding agents.">
  <meta property="og:url" content="https://navanithans.github.io/Agent-Skill-Kit/docs/">
  <meta property="og:image" content="https://raw.githubusercontent.com/NavanithanS/Agent-Skill-Kit/master/assets/banner.png">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="Skill browser &amp; command builder — Agent Skill Kit">
  <meta name="twitter:description" content="Browse {len(skills)} agent skills and build ask commands for five coding agents.">
  <meta name="twitter:image" content="https://raw.githubusercontent.com/NavanithanS/Agent-Skill-Kit/master/assets/banner.png">
...
<!-- ====== STATIC INDEX ======
     Rendered server-side as real anchors. The interactive browser above is
     built by JS from the SKILLS array, which crawlers may never execute; this
     block is what makes the library crawlable and gives every per-skill
     Jekyll page an internal link. Do not replace it with JS rendering. -->
<section id="all-skills" class="py-14 px-4 border-t border-gray-100 dark:border-zinc-800 dark:bg-zinc-950">
  <div class="max-w-6xl mx-auto">
    <h2 class="text-xl font-bold text-gray-900 dark:text-zinc-50 mb-2">All {skill_count} skills</h2>
    <p class="text-sm text-gray-500 dark:text-zinc-400 mb-6">
      Every skill in the library, with a direct link to its reference page. Each installs to
      Claude Code, Codex, Gemini CLI, Cursor or Antigravity with a single <code>ask copy</code> command.
    </p>
    {static_skill_index}
  </div>
</section>
...
<footer class="border-t border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 py-8 px-4">
  <div class="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
    ...
    <nav class="flex items-center gap-4 text-sm text-gray-400">
      <a href="https://navanithans.github.io/Agent-Skill-Kit/" class="hover:text-gray-700 transition-colors">Documentation</a>
      <a href="https://github.com/NavanithanS/Agent-Skill-Kit" class="hover:text-gray-700 transition-colors">GitHub&nbsp;↗</a>
      <a href="https://pypi.org/project/agent-skill-kit/" class="hover:text-gray-700 transition-colors">PyPI&nbsp;↗</a>
    </nav>
  </div>
</footer>
...
def main():
    ...
    # Crawlable hub page at /skills/. Gives every per-skill Jekyll page a
    # parent with a real internal link; without it those pages are orphans.
    (ROOT / "skills" / "README.md").write_text(
        generate_skill_index(skills), encoding="utf-8"
    )

    # Keep the README's skill table in sync with the library. Failing loudly
    # matters: docs.yml commits README.md unconditionally, so a silent skip
    # would ship a permanently stale table behind a green build.
    if not sync_readme_table(skills):
        sys.exit(1)
