...
> **📖 [Documentation & skill reference](https://navanithans.github.io/Agent-Skill-Kit/)** · **[Interactive command builder](https://navanithans.github.io/Agent-Skill-Kit/docs/)**
...
*Create once. Share across agents.* See [CHANGELOG.md](CHANGELOG.md) for release notes.
...
All skills, with a link to each one's reference page. Full index: **[browse the skill library](https://navanithans.github.io/Agent-Skill-Kit/skills/)**.
...
|---|---|---|
| [`ask-bug-finder`](skills/coding/ask-bug-finder/) | coding | Best practices for systematic bug hunting and debugging |
| [`ask-code-reviewer`](skills/coding/ask-code-reviewer/) | coding | An AI code reviewer that provides constructive feedback on code changes. |
| [`ask-commit-assistance`](skills/coding/ask-commit-assistance/) | coding | Assist with code review and staging. NEVER automatically commits. |
| [`ask-component-scaffolder`](skills/coding/ask-component-scaffolder/) | coding | Standardizes UI component creation by generating a consistent folder structure and files. |
| [`ask-conceptual-integrity-sentinel`](skills/coding/ask-conceptual-integrity-sentinel/) | coding | Principal-level engineering agent that audits repositories for architectural drift, bloated abstractions, and "dead code." |
...
| [`ask-impact-sentinel`](skills/coding/ask-impact-sentinel/) | coding | Guidelines for impact analysis, breaking change detection, strategic database design, and comprehensive database indexing review. |
| [`ask-laravel-architect`](skills/coding/ask-laravel-architect/) | coding | Senior scaffolding skill. Handles SQL vs Mongo (Jenssegers/Official), SoftDeletes, and strict API standards. |
| [`ask-laravel-mechanic`](skills/coding/ask-laravel-mechanic/) | coding | Senior maintenance skill. Enforces "Zero Data Loss" policies and handles Mongo/SQL debugging. |
| [`ask-nextjs-architect`](skills/coding/ask-nextjs-architect/) | coding | Expert scaffolding for Next.js 14+ (App Router). Enforces Server Components, Server Actions, and SEO best practices. |
| [`ask-owasp-security-review`](skills/coding/ask-owasp-security-review/) | coding | Conduct a thorough static security review of code, identifying vulnerabilities aligned with OWASP Top 10 risks, with severity ratings and remediation suggestions. |
| [`ask-python-refactor`](skills/coding/ask-python-refactor/) | coding | Best practices and guidelines for Python code refactoring |
| [`ask-readme-gardener`](skills/coding/ask-readme-gardener/) | coding | Keeps documentation in sync with code by updating the README.md when features or APIs are added. |
| [`ask-refactoring-readability`](skills/coding/ask-refactoring-readability/) | coding | Refactor code to enhance readability, following principles like DRY, meaningful names, and modularization. |
...
| [`ask-adr-logger`](skills/planning/ask-adr-logger/) | planning | Automatically records Architectural Decision Records (ADRs) when a significant technical decision is made. |
| [`ask-brainstorm`](skills/planning/ask-brainstorm/) | planning | Explores user intent, requirements, and design before implementation. Required before any creative work. |
| [`ask-buildmaster`](skills/planning/ask-buildmaster/) | planning | Smart Epic Orchestration Agent - Acts as PM + Tech Lead + Delivery Manager for epic planning, execution, and delivery. |
| [`ask-hold-code`](skills/planning/ask-hold-code/) | planning | Planning-mode lock that prevents code generation until the user explicitly approves implementation. |
| [`ask-project-memory`](skills/planning/ask-project-memory/) | planning | Maintains a 'Project Brain' by recording architectural decisions and tech stack choices in a memory file. |
| [`ask-solution-architect`](skills/planning/ask-solution-architect/) | planning | Master Ideation and Strategic Architecture skill. Executes professional, multi-perspective ideation sessions utilizing SCAMPER, Six Hats, and Design Thinking. |
| [`ask-add-agent`](skills/tooling/ask-add-agent/) | tooling | How to add support for new AI code editors to Agent Skill Kit |
| [`ask-ast-mapper`](skills/tooling/ask-ast-mapper/) | tooling | Read-only subagent designed to generate lightweight AST (Abstract Syntax Tree) dependency maps. |
| [`ask-context-janitor`](skills/tooling/ask-context-janitor/) | tooling | Aggressive token optimizer and context summarizer for AI orchestrators |
| [`ask-parallel-auditor`](skills/tooling/ask-parallel-auditor/) | tooling | Orchestrator skill that splits a target repository into chunks and runs multiple audit subagents in parallel. |
| [`ask-pdf-processing`](skills/tooling/ask-pdf-processing/) | tooling | PDF text extraction, form filling, and merging using pypdf and pdfplumber |
| [`ask-skill-capture`](skills/tooling/ask-skill-capture/) | tooling | Meta-skill. Analyzes the current session's lessons and saves them as a permanent reusable skill. |
| [`ask-skill-creator`](skills/tooling/ask-skill-creator/) | tooling | Teaches AI agents how to create skills for Agent Skill Kit |
| [`ask-smart-booking-test`](skills/tooling/ask-smart-booking-test/) | tooling | An advanced autonomous testing skill for verifying end-to-end booking flows. Specializes in Flights, Movies, and Tours with integrated Payment Gateway testing. |
| [`ask-system-architect-prime`](skills/tooling/ask-system-architect-prime/) | tooling | Principal Software Architect for repository audits, complexity analysis, and actionable refactoring recommendations |
| [`ask-wiki-init`](skills/tooling/ask-wiki-init/) | tooling | Scaffolds the LLM Wiki pattern into any project — a persistent, compounding knowledge base maintained by AI agents. |
